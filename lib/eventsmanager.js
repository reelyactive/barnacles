...
var DEFAULT_ACCEPT_STALE_EVENTS = false;
var DEFAULT_ACCEPT_FUTURE_EVENTS = true;
...
var REELCEIVER_STATISTICS_EVENT_TYPE = 'reelceiverStatistics';
...
function EventsManager(options) {
  ...
  self.acceptStaleEvents = options.acceptStaleEvents ||
                           DEFAULT_ACCEPT_STALE_EVENTS;
  self.acceptFutureEvents = options.acceptFutureEvents ||
                            DEFAULT_ACCEPT_FUTURE_EVENTS;
  self.db = new nedb();
...
EventsManager.prototype.handleTiraid = function(tiraid, callback) {
  ...
  var isStale = reelib.time.isInPast(reelib.tiraid.getTimestamp(tiraid) +
                                     self.disappearanceMilliseconds);
  var isFuture = !reelib.time.isInPast(reelib.tiraid.getTimestamp(tiraid));

  if((isStale && self.acceptStaleEvents) ||
     (isFuture && self.acceptFutureEvents)) {
    tiraid.timestamp = reelib.time.getCurrentISO(); // Adjust timestamp
  }
  else if(isStale || isFuture) {
    return callback();                              // Reject tiraid
  }
...
function createEventTiraid(instance, device, type) {
  ...
  // Create tiraid from recent decodings
    ...
    var identifier = getLatestDeviceIdentifier(device);
    ...
    var associationIds = mergeAssociationIds(device);
...
/**
 * Get the latest device identifier.
 * @param {Object} device The device.
 * @return {Object} The latest identifier.
 */
function getLatestDeviceIdentifier(device) {
  if(device.payloads.length > 0) {
    var lastPayloadIndex = device.payloads.length - 1;
    return device.payloads[lastPayloadIndex].identifier;
  }
  return device.lastEvent.identifier;
}


/**
 * Merge the association ids from the device.
 * @param {Object} device The device.
 * @return {Array} The merged array of association ids.
 */
function mergeAssociationIds(device) {
  var associationIds = reelib.tiraid.getAssociationIds(device.lastEvent);

  if(device.payloads.length > 0) {
    for(var cPayload = 0; cPayload < device.payloads.length; cPayload++) {
      var ids = reelib.tiraid.getAssociationIds(device.payloads[cPayload]);
      for(var cId = 0; cId < ids.length; cId++) {
        if(associationIds.indexOf(ids[cId]) < 0) {
          associationIds.push(ids[cId]);
        }
      }
    }
  }
...
/**
 * Handle a reel event.
 * @param {Object} data The reel data object.
 * @param {callback} callback Function to call on completion.
 */
EventsManager.prototype.handleReelEvent = function(data, callback) {
  var self = this;

  if(data && data.hasOwnProperty('type')) {
    switch(data.type) {
      case REELCEIVER_STATISTICS_EVENT_TYPE:
        self.emit(REELCEIVER_STATISTICS_EVENT_TYPE, data);
        break;
      default:
    }
  }
