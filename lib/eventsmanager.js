...
EventsManager.prototype.handleTiraid = function(tiraid, callback) {
  ...
  this.db.update({ _id: id, "tiraid.radioDecodings.0.identifier.value": strongestDecoder },
    ...
    // Nothing changed, this is effectively a Keep-Alive
    else {
      callback( { event: "keep-alive" } );
    }
  });
};
  ...
  if(!hasIds) {
    ...
    switch(options.query) {
      ...
      case 'receivedStrongestBy':
        self.db.find({ "tiraid.radioDecodings.0.identifier.value":
                         { $in: options.ids } }, {})
               .sort({ "tiraid.identifier.value": 1 })
