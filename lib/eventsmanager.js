...
/**
 * Detect timeout events (disappearance and keep-alive) from the database
 * @param {EventsManager} instance The given instance.
 */
function detectTimeoutEvents(instance) {
  ...
  instance.db.find({ $or: [ { "tiraid.timestamp": { $lt: disappearanceCutoffTime } },
    ...
    for(var cDoc = 0; cDoc < docs.length; cDoc++) {
      ...
      var eventTimestamp = doc.eventTimestamp;
      var isDisappearance = (tiraid.timestamp < disappearanceCutoffTime);
      var isKeepAlive = (eventTimestamp < keepAliveCutoffTime);
      if(isDisappearance) {
        instance.db.remove({ _id: id }, {}, function(err, numRemoved) {
          instance.emit('disappearance', tiraid);
          instance.currentStatistics.disappearances++;
        });
      }
      else if(isKeepAlive) { // TODO: make an array of keep-alive IDs and update batch?
        instance.db.update({ _id: id }, { $set: { eventTimestamp: currentTime } },
                           {}, function(err, numReplaced) {
