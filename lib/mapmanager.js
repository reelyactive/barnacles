...
class MapManager {
  ...
  retrieveContext(deviceIds, depth, callback) {
    ...
    // Step 1: retrieve all the active first-level devices
    if(isAllDevices) {
      for(const signature of self.store.keys()) {
        let device = self.store.get(signature);
      ...
      deviceIds.forEach(function(item) {
        let signature = item.deviceId + Raddec.identifiers.SIGNATURE_SEPARATOR +
                        item.deviceIdType;
        let isDevicePresent = self.store.has(signature);
