...
class Device {
  ...
  assemble(properties) {
    ...
    if(isPropertySubset) {
      ...
      if(self.dynamb) {
        let isValidDynamb = removeStaleDynambProperties(self);
        if(isValidDynamb) {
  ...
  determineEvents(parameters, handleEvent) {
    ...
    // Special case for non-transmitters which have no raddec data:
    //   'disappear' in the absence of valid dynamb data.
    if(!self.isTransmitter) {
      let isValidDynamb = removeStaleDynambProperties(self);
      return isValidDynamb ? parameters.disappearanceMilliseconds : -1;
    }
...
function removeStaleDynambProperties(device) {
  ...
  if(!device.hasOwnProperty('dynamb') ||
     !device.hasOwnProperty('dynambTimeouts')) {
    return false;
  }
  ...
  let isValidDynamb = (Object.keys(device.dynamb).length > 0)

  return isValidDynamb;
}
