...
function updateState(instance, events, currentTime, callback) {
  if(events.length > 0) {
    ...
    if(event.type !== null) {
      update.$set.latestEvent = { type: event.type, raddec: event.raddec };
    }
...
function handleTimeoutEvent(instance, transmitter, currentTime) {
  ...
  event.nextTimeout = transmitter._status.timeout +
                      instance.keepAliveMilliseconds;

  if(event.type) {
    event.raddec = createEventRaddec(instance, transmitter);
 ...
 */
function createEventRaddec(instance, transmitter) {
  ...
  transmitter.raddecs.forEach(function(raddec) {
    ...
    if(isWithinTimeWindow) {
      mergeRssiSignatures(rssiSignature, raddec.rssiSignature);
    } 
  });
...
/**
 * Merge the given RSSI signatures.
 * @param {Array} toSignature The signature in which to merge.
 * @param {Array} fromSignature The signature from which to merge.
 */
function mergeRssiSignatures(toSignature, fromSignature) {
  fromSignature.forEach(function(fromEntry) {
    let foundMatch = false;
    toSignature.forEach(function(toEntry) {
      let isMatch = Raddec.identifiers.areMatch(fromEntry.receiverId,
                                                fromEntry.receiverIdType,
                                                toEntry.receiverId,
                                                toEntry.receiverIdType);
      if(isMatch) {
        foundMatch = true;
        let rssiSum = (fromEntry.rssi * fromEntry.numberOfDecodings) +
                      (toEntry.rssi * toEntry.numberOfDecodings);
        let numberOfDecodings = fromEntry.numberOfDecodings +
                                toEntry.numberOfDecodings;
        let rssi = Math.round(rssiSum / numberOfDecodings);
        toEntry.rssi = rssi;
        toEntry.numberOfDecodings = numberOfDecodings;
      }
    });
    if(!foundMatch) {
      toSignature.push(fromEntry);
    }
  });
  return toSignature;  // TODO: sort by RSSI!
}
