...
class RaddecManager extends EventEmitter {
  ...
  handleRaddec(raddec) {
    ...
    let timestamp = raddec.timestamp || raddec.initialTime || currentTime;

    raddec.timestamp = raddec.timestamp || timestamp;
 ...
 * @param {Array} observedEvents The types of events to observe.
 */
function determineNewStatus(isNew, previousStatus, raddecs,
                            delayMilliseconds, observedEvents) {
  let isNewPacket = false;
  let isPossibleDisplacement = false;
  ...
  if(observedEvents.includes(Raddec.events.PACKETS)) {
    isNewPacket = determineNewPacket(raddecs);
  }
  if(observedEvents.includes(Raddec.events.DISPLACEMENT)) {
    isPossibleDisplacement = determinePossibleDisplacement(raddecs);
  }
  ...
  return {
      ...
      isPossibleDisplacement: isPossibleDisplacement,
      timeout: timeout,
      latestEventTimeout: previousStatus.latestEventTimeout
  };
}
...
function determineNewPacket(raddecs) {
  ...
  // TODO: check against latestEvent
...
function handleTimeoutEvent(instance, transmitter, currentTime) {
  let event = { _id: transmitter._id, timeout: transmitter._status.timeout };
  let events = [];
  let isObserved = false;

  if(transmitter._status.isAppearance) {
    events.push(Raddec.events.APPEARANCE);
    isObserved = instance.observedEvents.includes(Raddec.events.APPEARANCE);
  }
    ...
    isObserved = instance.observedEvents.includes(Raddec.events.DISPLACEMENT);
  }
  else if(isKeepAlive(instance, transmitter, currentTime)) {
    events.push(Raddec.events.KEEPALIVE);
    isObserved = instance.observedEvents.includes(Raddec.events.KEEPALIVE);
  }

  if(transmitter._status.isNewPacket) {
    events.push(Raddec.events.PACKETS);
    isObserved = isObserved ||
                 instance.observedEvents.includes(Raddec.events.PACKETS);
  }
  ...
  else if(instance.observedEvents.includes(Raddec.events.KEEPALIVE)) {
    event.timeout = transmitter._status.latestEventTimeout ||
                    transmitter._status.timeout;
  }

  event.nextTimeout = event.timeout + instance.keepAliveMilliseconds;
...
function createEventRaddec(instance, transmitter, events) {
  ...
  let earliestDecodingTime = latestTime -
                             instance.decodingCompilationMilliseconds;
  let earliestPacketTime = latestTime - instance.packetCompilationMilliseconds;
  ...
  transmitter.raddecs.forEach(function(raddec) {
    let isWithinDecodingTimeWindow = (raddec.time >= earliestDecodingTime);
    if(isWithinDecodingTimeWindow) {
      Raddec.mergeRssiSignatures(raddec.rssiSignature, rssiSignature);
    } 

    let isWithinPacketTimeWindow = (raddec.time >= earliestPacketTime);
    if(isWithinPacketTimeWindow) {
...
function isDisplacement(instance, transmitter) {
  ...
  let isReceiverChange = !Raddec.identifiers.areMatch(previousReceiverId,
