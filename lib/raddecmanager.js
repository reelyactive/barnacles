...
function handleTimeoutEvent(instance, transmitter, currentTime) {
  ...
  if(event.type) {
    ...
    // TODO: this should be implicitly handled earlier
    switch(event.type) {
      case 'appearance':
        event.raddec.addEvent(Raddec.events.APPEARANCE);
        break;
      case 'displacement':
        event.raddec.addEvent(Raddec.events.DISPLACEMENT);
        break;
      case 'packets':
        event.raddec.addEvent(Raddec.events.PACKETS);
        break;
      case 'keep-alive':
        event.raddec.addEvent(Raddec.events.KEEPALIVE);
        break;
    }
