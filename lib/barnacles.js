...
function handleRaddecManagerEvents(instance) {
  instance.raddecManager.on('appearance', function(raddec) {
    outputRaddec(instance, 'appearance', raddec);
  });
  instance.raddecManager.on('displacement', function(raddec) {
    outputRaddec(instance, 'displacement', raddec);
  });
  instance.raddecManager.on('packets', function(raddec) {
    outputRaddec(instance, 'packets', raddec);
  });
  instance.raddecManager.on('keep-alive', function(raddec) {
    outputRaddec(instance, 'keep-alive', raddec);
  });
}
 ...
 */
function outputRaddec(instance, event, raddec) {
  raddec.event = event; /* TODO: how best to include event type in raddec? */
