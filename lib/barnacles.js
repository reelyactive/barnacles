...
Barnacles.prototype.createWebSocket = function(options) {
  ...
  if(options.server) {
    ...
    if(self.enableMasterSocketForwarding) {
      handleOutboundSocketEvents(self);
    }
  }
};
...
Barnacles.prototype.bind = function(options) {
  ...
  if(options.barnowl) {
    ...
    options.barnowl.on('infrastructureEvent', function(data) {
      self.eventsManager.handleInfrastructure(data, function(){});
    });
  }    
};
...
Barnacles.prototype.addService = function(options) {
  ...
  switch(options.service) {
    case "barnaclesrest":
      self.services.barnaclesrest = new barnaclesrest( 
        { eventsManager: self.eventsManager,
          ...
          ignoreInfrastructureTx: options.ignoreInfrastructureTx }
      );
      break;
    case "google":
    ...
    case "initialstate":
      ...
      break;
    case "logfile":
      self.services.logfile = new logfile( 
        { eventsManager: self.eventsManager,
...
function handleInboundSocketEvents(instance, socket) {
  socket.on('appearance', function(tiraid) {
    instance.eventsManager.handleEvent('appearance', tiraid,
                                       function(data, err) {});
  });
  socket.on('displacement', function(tiraid) {
    instance.eventsManager.handleEvent('displacement', tiraid,
                                       function(data, err) {});
  });
  socket.on('disappearance', function(tiraid) {
    instance.eventsManager.handleEvent('disappearance', tiraid,
                                       function(data, err) {});
  });
  socket.on('keep-alive', function(tiraid) {
    instance.eventsManager.handleEvent('keep-alive', tiraid,
                                       function(data, err) {});
  });
}
...
/**
 * Handle all events to be emitted by the websocket
 * @param {Barnacles} instance The given instance.
 */
function handleOutboundSocketEvents(instance) {
  instance.eventsManager.on('appearance', function(tiraid) {
    instance.io.emit('appearance', { tiraid: tiraid });
  });
  instance.eventsManager.on('displacement', function(tiraid) {
    instance.io.emit('displacement', { tiraid: tiraid });
  });
  instance.eventsManager.on('disappearance', function(tiraid) {
    instance.io.emit('disappearance', { tiraid: tiraid });
  });
  instance.eventsManager.on('keep-alive', function(tiraid) {
    instance.io.emit('keep-alive', { tiraid: tiraid });
  });
}
