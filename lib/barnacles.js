...
var DEFAULT_INCLUDE_TIRAID_IN_EVENT = false;
var DEFAULT_POSITIONING_METHOD = 'strongestReceiver';
...
function Barnacles(options) {
  ...
  self.includeTiraidInEvent = options.includeTiraidInEvent ||
                              DEFAULT_INCLUDE_TIRAID_IN_EVENT;
 ...
 * @param {Boolean} includeTiraid Include tiraid in event?
 */
function flattenEvent(type, tiraid, state, includeTiraid) {
  var flattened = {};
  ...
  if(receiver.position) {
    flattened.position = receiver.position;
    flattened.positioningMethod = DEFAULT_POSITIONING_METHOD;
  }
  if(includeTiraid) {
    flattened.tiraid = tiraid; // Legacy support
  }
