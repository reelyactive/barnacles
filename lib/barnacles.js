  ...
  if(options.server) {
    ...
    self.io.on('connection', function(socket) {
      socket.on('appearance', function(tiraid) {
        self.eventsManager.handleEvent('appearance', tiraid,
                                       function(data, err) {});
      });
      socket.on('keep-alive', function(tiraid) {
        self.eventsManager.handleEvent('keep-alive', tiraid,
                                       function(data, err) {});
      });
      socket.on('displacement', function(tiraid) {
        self.eventsManager.handleEvent('displacement', tiraid,
                                       function(data, err) {});
      });
      socket.on('disappearance', function(tiraid) {
        self.eventsManager.handleEvent('disappearance', tiraid,
                                       function(data, err) {});
      });
    });
